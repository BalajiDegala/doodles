...
| File | Purpose |
| --- | --- |
| [Namespace](manifests/00-namespace.yaml) | Creates `k8s-learning-platform` with a library ownership label |
| [Page and metrics](manifests/10-page.yaml) | Holds the visible HTML and a fixed teaching metric |
| [Deployment](manifests/20-deployment.yaml) | Maintains two `platform-catalog` web replicas |
