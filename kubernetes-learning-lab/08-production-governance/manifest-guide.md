...
| File | Purpose |
| --- | --- |
| [Namespace](manifests/00-namespace.yaml) | Isolates the chapter in `k8s-learning-governance` |
| [Page](manifests/10-page.yaml) | Supplies the static catalog HTML and teaching metric |
| [Deployment](manifests/20-deployment.yaml) | Maintains two healthy `governance-catalog` replicas |
| [Service](manifests/30-service.yaml) | Maps port 80 to named container port `http`, 8080 |

The Deployment selector, Pod label, and Service selector agree on `app: governance-catalog`. The server reads its ConfigMap at `/www`. Each replica requests 25m CPU and 32Mi memory, with limits of 100m and 64Mi. The health probe reads `/`; rollout settings permit one surge replica and zero deliberately unavailable replicas.
